...
import nn_implementation as nn
import pca as pca
...
def train_neural_network(x, act_fnctn, act_diff, activation_dif, no_hidden_units):
    batch_size = 400
    no_input_units = 100 # number of input units of should be 192 * 168 = 32256(S_1 = 32256)
    ...
    learning_rate = 0.001
    T = 2000 # using iteration instead of convergence
    for it in range(T):
        print(it)
        w_1_iter = 0
        b_1_iter = 0
...
if __name__ == "__main__":
    ...
    #first apply pca to reduce d=100
    print ("Dimension reduction calculating")
    u, x_train = pca.pca(x_train_full, 100)
    x_test = u.T*x_train
    ...
    clf = LinearSVC()
    clf.fit(x_train.T, y_train_1)

    print("SVM accuracy is: ",clf.score((u.T * x_test).T, y_test_1))

    from sklearn import linear_model

    logreg = linear_model.LogisticRegression(C=1e5)
    logreg.fit(x_train.T, y_train_1)
    print("The Logistic regression accuracy is: ",logreg.score((u.T * x_test).T, y_test_1))
